...
# Ensure UTF-8 output to prevent garbled characters in Windows terminals
...
if (config.apiKey && config.model) {
  process.env.ANTHROPIC_MODEL ??= resolveAlias(config.model);
}
if (config.apiKey && config.smallModel) {
  process.env.ANTHROPIC_SMALL_FAST_MODEL ??= resolveAlias(config.smallModel);
}
if (config.timeoutMs) {

...
// Load model aliases for third-party API endpoints
...
const patches = [
  ...
  },
  {
    ...
    // Drop the firstParty/anthropicAws gate AND model name check so any model can use auto-mode
    ...
    replacer: (m, v, modelVar) => `return!0`,
  },
  // ── 绿色主题 (patch 标识) ──
...
if (revert) {
  if (!existsSync(BACKUP)) { console.error('[ERR] No backup found'); process.exit(1); }
...
if (!existsSync(TARGET)) {
  console.error('[ERR] Target not found:', TARGET);
  process.exit(1);
}
...
for (const p of patches) {
  ...
  if (p.unique && relevant.length !== 1) {
    if (verify) {
      ...
      console.log(`  [WARN] ${p.name} - ${relevant.length} matches, skipping (need 1)`);
    }
  ...
  if (relevant.length === 0) {
    if (p.optional) {
      console.log(`  [SKIP] ${p.name} (not present in this version)`);
      skipped++;
    } else {
  ...
  if (verify) {
    console.log(`  [PENDING] ${p.name} - ${relevant.length} match(es), not yet applied`);
    skipped++;
    continue;
  }
  ...
  if (count > 0) {
    ...
    console.log(`  [SKIP] ${p.name} (no change needed)`);
    skipped++;
  }
}
...
if (!dryRun && !verify && applied > 0) {
  ...
  console.log(`  [Write] cli.original.js (${diff >= 0 ? '+' : ''}${diff} bytes)`);
}
